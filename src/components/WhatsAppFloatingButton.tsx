...
export const WhatsAppFloatingButton: React.FC = () => {
  ...
  // Show floating tooltip prompt after a gentle delay
  useEffect(() => {
  ...
  const displayName = personalInfo?.shortName || personalInfo?.name || 'Md. Habibul Bashar';
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 no-print flex flex-col items-end" ref={popupRef}>
      ...
      {showPrompt && !isOpen && (
        <div
          className="mb-3 max-w-[250px] sm:max-w-xs bg-white dark:bg-slate-900 border border-emerald-500/30 rounded-2xl p-3 shadow-xl shadow-emerald-500/10 flex items-start gap-2.5 cursor-pointer hover:border-emerald-500 transition-all duration-300 animate-in fade-in slide-in-from-bottom-3"
          onClick={() => {
            setIsOpen(true);
            setShowPrompt(false);
          }}
      ...
      {isOpen && (
        <div
          className="mb-4 w-[calc(100vw-2rem)] sm:w-[360px] rounded-3xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col transition-all duration-300 animate-in fade-in zoom-in-95 slide-in-from-bottom-4"
        >
